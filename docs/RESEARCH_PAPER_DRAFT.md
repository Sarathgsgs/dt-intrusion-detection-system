...
**Status:** Complete Empirical Draft (Post-Revision v4)

---

## Abstract
Modern Industrial Internet of Things (IIoT) architectures require intrusion detection systems (IDS) that deliver high detection accuracy and explainable, physically grounded threat attributions while operating within strict edge-resource boundaries. In this paper, we propose **Twin-Guided Explainable Intrusion Detection System (X-IDS)**. X-IDS couples a scope-restricted neural sequence Digital Twin trained exclusively on normal operational dynamics with strict physical domain bounding, a targeted deviation engine, SHAP local feature attribution, and an operational confidence filter. Crucially, by partitioning telemetry into continuous physical dynamics versus discrete protocol states and bounding forecasts to physical limits, we eliminate noise injection in sequence forecasting. We benchmark four distinct edge deployment configurations, evaluating detection accuracy, Macro-F1, inference latency, memory footprint, fine-grained 15-class per-attack performance, and cross-dataset zero-shot generalization on the TON_IoT testbed. 

Our empirical results show that while an ultra-lightweight raw-feature XGBoost configuration (Config 4) achieves ultra-fast inference ($0.006\text{ ms/sample}$, $105.9\text{ KB}$ footprint, $91.81\%$ accuracy), the Quantized Digital Twin configuration (Config 2) achieves $92.92\%$ accuracy ($0.196\text{ ms/sample}$, $6.64\text{ MB}$ footprint) with physically verifiable residual vectors. Fine-grained 15-class evaluation reveals exact or statistical parity across 10 of 15 attack types—including perfect $1.0000\text{ F1}$ on volumetric DDoS floods and zero false-alarm baseline fidelity. Crucially, the Digital Twin provides causal physical grounding and enables an operational confidence filter that suppresses $30.0\%$ ($28.6\% - 31.4\%$) of ambiguous false alarms, solving the alert fatigue bottleneck in safety-critical industrial networks.
...
2. **Targeted Deviation Fusion:** Combining continuous residual vectors ($|y_t - \hat{y}_t|$) with raw discrete states to form an augmented feature space ($\mathbb{R}^{43}$), achieving $94.81\%$ accuracy (within 0.19% of the baseline) while boosting pure continuous residual detection to $62.30\%$.
3. **Fine-Grained 15-Class Per-Attack Analysis:** Demonstrating exact parity on volumetric floods ($F_1 = 1.0000$) and restoring Random Forest capability on application attacks (`Uploading` $F_1 = 0.9002$, `SQL_injection` $F_1 = 0.8601$).
...
|---|---|---|---|---|---|---|---|
| **XSS** | Application / Payload | 892 | **0.9084** | **0.9085** | `+0.0001` | **0.8839** | `Statistical Parity` |
...
| **DDoS_ICMP** | Volumetric Flood | 1250 | **0.9996** | **0.9996** | `0.0000` | **0.9992** | `Exact Parity` |
| **Backdoor** | Application / Payload | 904 | **0.9848** | **0.9848** | `0.0000` | 0.9758 | `Statistical Parity` |
| **Normal** | Healthy Baseline | 2156 | **0.9979** | **0.9977** | `-0.0002` | **0.9977** | `Exact Parity` |
| **MITM** | Stealth Behavioral | 108 | 0.5806 | 0.5792 | `-0.0015` | **0.5887** | `Twin RF Advantage` |
| **Uploading** | Application / Payload | 911 | **0.9221** | **0.9205** | `-0.0016` | 0.9002 | `Statistical Parity` |
| **Vulnerability_scanner** | Application / Payload | 894 | **0.9759** | 0.9730 | `-0.0028` | 0.9737 | `Statistical Parity` |
| **Password** | Application / Payload | 886 | **0.8990** | 0.8953 | `-0.0037` | 0.8675 | `Statistical Parity` |
| **SQL_injection** | Application / Payload | 915 | **0.8963** | 0.8901 | `-0.0062` | 0.8601 | `Raw Preferred` |
| **DDoS_HTTP** | Volumetric Flood | 937 | **0.8571** | 0.8507 | `-0.0065` | 0.8274 | `Raw Preferred` |
| **Port_Scanning** | Volumetric / Recon | 893 | **0.9511** | 0.9444 | `-0.0068` | 0.9374 | `Raw Preferred` |
| **Fingerprinting** | Stealth Recon | 89 | **0.8889** | 0.8750 | `-0.0139` | 0.8625 | `Raw Preferred` |
| **Ransomware** | Application / Payload | 969 | **0.9385** | 0.9176 | `-0.0209` | 0.9190 | `Raw Preferred` |
...
1. **When to Deploy Config 4 (Lightweight XGBoost):**
   - For resource-constrained sensor microcontrollers and high-volume edge gateways requiring maximum throughput ($>150,000\text{ samples/sec}$) and minimal memory footprint ($105.9\text{ KB}$), Config 4 is the optimal design choice.
